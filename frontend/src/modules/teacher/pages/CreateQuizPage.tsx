...
const CreateQuizPage = () => {
  ...
  const { register, control, handleSubmit, formState: { errors, isSubmitting } } = useForm({
    defaultValues: {
      ...
      description: '',
      category: 'General',
      // Each question now defaults to 4 empty options
  ...
  const onSubmit = async (data) => {
    // Before sending, we need to transform the correctAnswer index into the actual answer string
    const processedData = {
      ...data,
      questions: data.questions.map(q => {
        // The value from the radio button is the index of the correct option
        const correctIndex = parseInt(q.correctAnswer, 10);
        return {
          ...q,
          // Set the correctAnswer to the text from the options array
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {fields.map((field, index) => (
            <div key={field.id} className="p-4 border rounded-md space-y-4 relative">
              ...
              <Input placeholder="Question text" {...register(`questions.${index}.text`, { required: "Question text is required" })} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {[0, 1, 2, 3].map((optionIndex) => (
                  <div key={optionIndex} className="flex items-center gap-2">
                    <input
                      type="radio"
                      {...register(`questions.${index}.correctAnswer`, { required: "Please select a correct answer" })}
                      value={optionIndex}
                      className="h-5 w-5"
                    />
                    <Input
                      placeholder={`Option ${optionIndex + 1}`}
                      {...register(`questions.${index}.options.${optionIndex}`, { required: "Option cannot be empty" })}
                    />
                  </div>
                ))}
              </div>
